...
export default function ApiSettingsPage() {
  ...
  // Mock session data - en producción esto vendría del contexto de sesión
  const sessionId = 'mock-session-id'
  const userId = 'mock-user-id'

  async function load() {
    try {
      const s = await api.providersStatus(sessionId, userId)
      setStatus(s)
    } catch (e: any) {
  ...
  return (
    <div className="app-container">
      ...
      <div className="card" style={{ display: 'grid', gap: 12, maxWidth: 560 }}>
        ...
        <div className="row">
          <button className="btn btn-primary" onClick={async () => { await api.connectProvider(sessionId, userId, apiKey, provider); showToast('Provider connected', 'success'); load() }}>Connect</button>
          <button className="btn" onClick={async () => { await api.disconnectProvider(sessionId, userId, provider); showToast('Provider disconnected', 'success'); load() }}>Disconnect</button>
        </div>
      </div>
